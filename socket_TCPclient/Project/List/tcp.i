...
#line 109 "..\\Lwip\\core\\tcp.c"
#line 110 "..\\Lwip\\core\\tcp.c"
#line 111 "..\\Lwip\\core\\tcp.c"
#line 112 "..\\Lwip\\core\\tcp.c"
#line 113 "..\\Lwip\\core\\tcp.c"
...
{
  do { if (!(pcb->state != LISTEN)) { do {printf("tcp_free: LISTEN");}while(0); }} while(0);
...
{
  do { if (!(pcb->state != LISTEN)) { do {printf("tcp_free_listen: !LISTEN");}while(0); }} while(0);
...
tcp_remove_listener(struct tcp_pcb *list, struct tcp_pcb_listen *lpcb)
{
  ...
  do { if (!(lpcb != 0)) { do {printf("tcp_remove_listener: invalid listener");}while(0); }} while(0);
...
tcp_listen_closed(struct tcp_pcb *pcb)
{
  ...
  size_t i;
  do { if (!(pcb != 0)) { do {printf("pcb != NULL");}while(0); }} while(0);
  do { if (!(pcb->state == LISTEN)) { do {printf("pcb->state == LISTEN");}while(0); }} while(0);
...
{
  do { if (!(pcb != 0)) { do {printf("tcp_close_shutdown: invalid pcb");}while(0); }} while(0);

  if (rst_on_unacked_data && ((pcb->state == ESTABLISHED) || (pcb->state == CLOSE_WAIT))) {
    if ((pcb->refused_data != 0) || (pcb->rcv_wnd != (14*(1500 - 40)))) {
      
 
      do { if (!(pcb->flags & 0x10U)) { do {printf("pcb->flags & TF_RXCLOSED");}while(0); }} while(0);
...
tcp_close(struct tcp_pcb *pcb)
{
  ...
  do { if (!(pcb != 0)) { do {printf("tcp_close: invalid pcb");}while(0); return ERR_ARG;}} while(0);
  ;
...
tcp_shutdown(struct tcp_pcb *pcb, int shut_rx, int shut_tx)
{
  ...
  do { if (!(pcb != 0)) { do {printf("tcp_shutdown: invalid pcb");}while(0); return ERR_ARG;}} while(0);
...
tcp_abandon(struct tcp_pcb *pcb, int reset)
{
  ...
  do { if (!(pcb != 0)) { do {printf("tcp_abandon: invalid pcb");}while(0); return;}} while(0);

   
  do { if (!(pcb->state != LISTEN)) { do {printf("don't call tcp_abort/tcp_abandon for listen-pcbs");}while(0); }} while(0);
...
tcp_bind(struct tcp_pcb *pcb, const ip_addr_t *ipaddr, u16_t port)
{
  ...
  do { if (!(pcb != 0)) { do {printf("tcp_bind: invalid pcb");}while(0); return ERR_ARG;}} while(0);

  do { if (!(pcb->state == CLOSED)) { do {printf("tcp_bind: can only bind in state CLOSED");}while(0); return ERR_VAL;}} while(0);

#line 695 "..\\Lwip\\core\\tcp.c"

#line 707 "..\\Lwip\\core\\tcp.c"
...
tcp_accept_null(void *arg, struct tcp_pcb *pcb, err_t err)
{
  ...
  do { if (!(pcb != 0)) { do {printf("tcp_accept_null: invalid pcb");}while(0); }} while(0);
...
tcp_listen_with_backlog_and_err(struct tcp_pcb *pcb, u8_t backlog, err_t *err)
{
  ...
  do { if (!(pcb != 0)) { do {printf("tcp_listen_with_backlog_and_err: invalid pcb");}while(0); res = ERR_ARG; goto done;}} while(0);
  do { if (!(pcb->state == CLOSED)) { do {printf("tcp_listen_with_backlog_and_err: pcb already connected");}while(0); res = ERR_CLSD; goto done;}} while(0);
...
tcp_update_rcv_ann_wnd(struct tcp_pcb *pcb)
{
  ...
  do { if (!(pcb != 0)) { do {printf("tcp_update_rcv_ann_wnd: invalid pcb");}while(0); }} while(0);
  new_right_edge = pcb->rcv_nxt + pcb->rcv_wnd;

  if (((s32_t)((u32_t)(new_right_edge) - (u32_t)(pcb->rcv_ann_right_edge + (((((14*(1500 - 40)) / 2)) < (pcb->mss)) ? (((14*(1500 - 40)) / 2)) : (pcb->mss)))) >= 0)) {
    ...
    if (((s32_t)((u32_t)(pcb->rcv_nxt) - (u32_t)(pcb->rcv_ann_right_edge)) > 0)) {
      ...
      do { if (!(new_rcv_ann_wnd <= 0xffff)) { do {printf("new_rcv_ann_wnd <= 0xffff");}while(0); }} while(0);
...
tcp_recved(struct tcp_pcb *pcb, u16_t len)
{
  ...
  do { if (!(pcb != 0)) { do {printf("tcp_recved: invalid pcb");}while(0); return;}} while(0);

   
  do { if (!(pcb->state != LISTEN)) { do {printf("don't call tcp_recved for listen-pcbs");}while(0); }} while(0);


  rcv_wnd = (tcpwnd_size_t)(pcb->rcv_wnd + len);
  if ((rcv_wnd > (14*(1500 - 40))) || (rcv_wnd < pcb->rcv_wnd)) {
     
    ;
    pcb->rcv_wnd = (14*(1500 - 40));
  } else  {
    pcb->rcv_wnd = rcv_wnd;
  }
  ...
  if (wnd_inflation >= (((((14*(1500 - 40)) / 4)) < (((1500 - 40) * 4))) ? (((14*(1500 - 40)) / 4)) : (((1500 - 40) * 4)))) {
            ...
            tcp_connected_fn connected)
{
  ...
  do { if (!(pcb != 0)) { do {printf("tcp_connect: invalid pcb");}while(0); return ERR_ARG;}} while(0);
  do { if (!(ipaddr != 0)) { do {printf("tcp_connect: invalid ipaddr");}while(0); return ERR_ARG;}} while(0);

  do { if (!(pcb->state == CLOSED)) { do {printf("tcp_connect: can only connect from state CLOSED");}while(0); return ERR_ISCONN;}} while(0);
...
#line 1142 "..\\Lwip\\core\\tcp.c"
  ...
  pcb->rcv_wnd = pcb->rcv_ann_wnd = (14*(1500 - 40));
  pcb->rcv_ann_right_edge = pcb->rcv_nxt;
  pcb->snd_wnd = (14*(1500 - 40));
...
tcp_slowtmr_start:
  ...
  while (pcb != 0) {
    ;
    do { if (!(pcb->state != CLOSED)) { do {printf("tcp_slowtmr: active pcb->state != CLOSED\n");}while(0); }} while(0);
    do { if (!(pcb->state != LISTEN)) { do {printf("tcp_slowtmr: active pcb->state != LISTEN\n");}while(0); }} while(0);
    do { if (!(pcb->state != TIME_WAIT)) { do {printf("tcp_slowtmr: active pcb->state != TIME-WAIT\n");}while(0); }} while(0);
    ...
    if (pcb->state == SYN_SENT && pcb->nrtx >= 6) {
      ...
      if (pcb->persist_backoff > 0) {
        do { if (!(pcb->unacked == 0)) { do {printf("tcp_slowtimr: persist ticking with in-flight data");}while(0); }} while(0);
        do { if (!(pcb->unsent != 0)) { do {printf("tcp_slowtimr: persist ticking with empty send buffer");}while(0); }} while(0);
        ...
        if (pcb->rtime >= pcb->rto) {
          ...
          if ((tcp_rexmit_rto_prepare(pcb) == ERR_OK) || ((pcb->unacked == 0) && (pcb->unsent != 0))) {
...
#line 1360 "..\\Lwip\\core\\tcp.c"
    ...
    if (pcb_remove) {
      ...
      if (prev != 0) {
        do { if (!(pcb != tcp_active_pcbs)) { do {printf("tcp_slowtmr: middle tcp != tcp_active_pcbs");}while(0); }} while(0);
        prev->next = pcb->next;
      } else {
         
        do { if (!(tcp_active_pcbs == pcb)) { do {printf("tcp_slowtmr: first pcb == tcp_active_pcbs");}while(0); }} while(0);
  ...
  pcb = tcp_tw_pcbs;
  while (pcb != 0) {
    do { if (!(pcb->state == TIME_WAIT)) { do {printf("tcp_slowtmr: TIME-WAIT pcb->state == TIME-WAIT");}while(0); }} while(0);
    pcb_remove = 0;
    ...
    if (pcb_remove) {
      ...
      if (prev != 0) {
        do { if (!(pcb != tcp_tw_pcbs)) { do {printf("tcp_slowtmr: middle tcp != tcp_tw_pcbs");}while(0); }} while(0);
        prev->next = pcb->next;
      } else {
         
        do { if (!(tcp_tw_pcbs == pcb)) { do {printf("tcp_slowtmr: first pcb == tcp_tw_pcbs");}while(0); }} while(0);
...
tcp_process_refused_data(struct tcp_pcb *pcb)
{
  ...
  do { if (!(pcb != 0)) { do {printf("tcp_process_refused_data: invalid pcb");}while(0); return ERR_ARG;}} while(0);
...
tcp_setprio(struct tcp_pcb *pcb, u8_t prio)
{
  ...
  do { if (!(pcb != 0)) { do {printf("tcp_setprio: invalid pcb");}while(0); return;}} while(0);

  pcb->prio = prio;
}

#line 1677 "..\\Lwip\\core\\tcp.c"
...
tcp_recv_null(void *arg, struct tcp_pcb *pcb, struct pbuf *p, err_t err)
{
  ...
  do { if (!(pcb != 0)) { do {printf("tcp_recv_null: invalid pcb");}while(0); return ERR_ARG;}} while(0);
...
tcp_kill_state(enum tcp_state state)
{
  ...
  do { if (!((state == CLOSING) || (state == LAST_ACK))) { do {printf("invalid state");}while(0); }} while(0);
...
tcp_recv(struct tcp_pcb *pcb, tcp_recv_fn recv)
{
  ;
  if (pcb != 0) {
    do { if (!(pcb->state != LISTEN)) { do {printf("invalid socket state for recv callback");}while(0); }} while(0);
    pcb->recv = recv;
  }
}
...
tcp_sent(struct tcp_pcb *pcb, tcp_sent_fn sent)
{
  ;
  if (pcb != 0) {
    do { if (!(pcb->state != LISTEN)) { do {printf("invalid socket state for sent callback");}while(0); }} while(0);
    pcb->sent = sent;
  }
}
...
tcp_err(struct tcp_pcb *pcb, tcp_err_fn err)
{
  ;
  if (pcb != 0) {
    do { if (!(pcb->state != LISTEN)) { do {printf("invalid socket state for err callback");}while(0); }} while(0);
    pcb->errf = err;
  }
}
...
tcp_poll(struct tcp_pcb *pcb, tcp_poll_fn poll, u8_t interval)
{
  ...
  do { if (!(pcb != 0)) { do {printf("tcp_poll: invalid pcb");}while(0); return;}} while(0);
  do { if (!(pcb->state != LISTEN)) { do {printf("invalid socket state for poll");}while(0); }} while(0);
...
{
  do { if (!(pcb != 0)) { do {printf("tcp_pcb_purge: invalid pcb");}while(0); return;}} while(0);
...
{
  do { if (!(pcb != 0)) { do {printf("tcp_pcb_remove: invalid pcb");}while(0); }} while(0);
  do { if (!(pcblist != 0)) { do {printf("tcp_pcb_remove: invalid pcblist");}while(0); }} while(0);
  ...
  if (pcb->state != LISTEN) {
    do { if (!(pcb->unsent == 0)) { do {printf("unsent segments leaking");}while(0); }} while(0);
    do { if (!(pcb->unacked == 0)) { do {printf("unacked segments leaking");}while(0); }} while(0);
  ...
  do { if (!(1)) { do {printf("tcp_pcb_remove: tcp_pcbs_sane()");}while(0); }} while(0);
}
...
tcp_next_iss(struct tcp_pcb *pcb)
{
  ...
  do { if (!(pcb != 0)) { do {printf("tcp_next_iss: invalid pcb");}while(0); }} while(0);
  (void)pcb;
...
tcp_eff_send_mss_netif(u16_t sendmss, struct netif *outif, const ip_addr_t *dest)
{
  ...
  do { if (!(dest != 0)) { do {printf("tcp_eff_send_mss_netif: invalid dst_ip");}while(0); }} while(0);

#line 2260 "..\\Lwip\\core\\tcp.c"
  {
    if (outif == 0) {
      return sendmss;
    }
...
tcp_netif_ip_addr_changed_pcblist(const ip_addr_t *old_addr, struct tcp_pcb *pcb_list)
{
  ...
  do { if (!(old_addr != 0)) { do {printf("tcp_netif_ip_addr_changed_pcblist: invalid old_addr");}while(0); }} while(0);
...
#line 2397 "..\\Lwip\\core\\tcp.c"

#line 2531 "..\\Lwip\\core\\tcp.c"

#line 2685 "..\\Lwip\\core\\tcp.c"
