...
slipif_output(struct netif *netif, struct pbuf *p)
{
  ...
  do { if (!((netif->state != 0))) { do {printf("netif->state != NULL");}while(0); }} while(0);
    ...
    for (i = 0; i < q->len; i++) {
      ...
      switch (c) {
        case 0xC0:
          ...
          sio_send(0xDB, priv->sd);
          sio_send(0xDC, priv->sd);
          break;
        case 0xDB:
           
          sio_send(0xDB, priv->sd);
          sio_send(0xDD, priv->sd);
          break;
        default:
...
slipif_rxbyte(struct netif *netif, u8_t c)
{
  ...
  if (priv->p == 0) {
    ...
    ;
    priv->p = pbuf_alloc(PBUF_LINK, (((((1500 - 40)+40+0+(14 + 0)) + 4 - 1U) & ~(4-1U)) - (14 + 0) - 0), PBUF_POOL);
  ...
  if ((priv->p != 0) && (priv->recved <= 1500)) {
    ...
    if (priv->i >= priv->p->len) {
      ...
      priv->i = 0;
      if (priv->p->next != 0 && priv->p->next->len > 0) {
...
slipif_init(struct netif *netif)
{
  ...
  do { if (!(netif->input != 0)) { do {printf("slipif needs an input callback");}while(0); }} while(0);

   
  sio_num = ((u8_t)((ptrdiff_t)netif->state));
  ...
  sys_thread_new("slipif_loop", slipif_loop_thread, netif,
                 0, 1);
