...
#line 46 "..\\Lwip\\core\\timeouts.c"

#line 48 "..\\Lwip\\core\\timeouts.c"
#line 49 "..\\Lwip\\core\\timeouts.c"
...
#line 60 "..\\Lwip\\core\\timeouts.c"
#line 1 "..\\Lwip\\include\\lwip/dhcp6.h"
...
#line 44 "..\\Lwip\\include\\lwip/dhcp6.h"

#line 103 "..\\Lwip\\include\\lwip/dhcp6.h"

#line 61 "..\\Lwip\\core\\timeouts.c"
#line 62 "..\\Lwip\\core\\timeouts.c"
#line 63 "..\\Lwip\\core\\timeouts.c"
...

{
  ...
  if (timeout == 0) {
    do { if (!(timeout != 0)) { do {printf("sys_timeout: timeout != NULL, pool MEMP_SYS_TIMEOUT is empty");}while(0); }} while(0);
    return;
  }
  ...
  if (( (((u32_t)((timeout->time)-(next_timeout->time))) > 0x7fffffff) ? 1 : 0 )) {
    timeout->next = next_timeout;
    next_timeout = timeout;
  } else {
    for (t = next_timeout; t != 0; t = t->next) {
      if ((t->next == 0) || ( (((u32_t)((timeout->time)-(t->next->time))) > 0x7fffffff) ? 1 : 0 )) {
        timeout->next = t->next;
        t->next = timeout;
        break;
      }
...
lwip_cyclic_timer(void *arg)
{
  ...
  if (( (((u32_t)((next_timeout_time)-(now))) > 0x7fffffff) ? 1 : 0 )) {
...
void sys_timeouts_init(void)
{
  ...
  for (i = (1 ? 1 : 0); i < (sizeof(lwip_cyclic_timers)/sizeof((lwip_cyclic_timers)[0])); i++) {
    ...
    sys_timeout(lwip_cyclic_timers[i].interval_ms, lwip_cyclic_timer, ((void *)((ptrdiff_t)&lwip_cyclic_timers[i])));
  }
}
...

{
  ...
  do { if (!(msecs <= (0xffffffff / 4))) { do {printf("Timeout time too long, max is LWIP_UINT32_MAX/4 msecs");}while(0); }} while(0);
...
sys_check_timeouts(void)
{
  ...
  do {
    ...
    if (( (((u32_t)((now)-(tmptimeout->time))) > 0x7fffffff) ? 1 : 0 )) {
      return;
    }
...
sys_timeouts_sleeptime(void)
{
  ...
  now = sys_now();
  if (( (((u32_t)((next_timeout->time)-(now))) > 0x7fffffff) ? 1 : 0 )) {
    return 0;
  } else {
    u32_t ret = (u32_t)(next_timeout->time - now);
    do { if (!(ret <= 0x7fffffff)) { do {printf("invalid sleeptime");}while(0); }} while(0);
    return ret;
  }
}
